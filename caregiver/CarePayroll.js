...
import { CarePayrollRequestPath } from "./constantCaregiver";

YellowBox.ignoreWarnings(["Remote debugger"]);

export default class CarePayroll extends React.Component {
  constructor(props) {
    super(props);
  }
  ...
  state = {
    ...
    rate: 0,
    token: "token",
    carePayroll: undefined,
  };

  getData = async () => {
    try {
      const value = await AsyncStorage.getItem("token");
      const payroll = await AsyncStorage.getItem("CarePayroll");
      // payroll = JSON.parse(payroll);
      if (value !== null && payroll !== null) {
        // value previously stored
        console.log("Token", value, payroll);
        this.setState({
          token: value,
          carePayroll: payroll,
        });
        console.log("carePayroll State:", this.state.carePayroll);
      }
    } catch (e) {
      // error reading value
  ...
  render() {
    ...
    console.log("Caregiver Props ID:", this.state.carePayroll);
    const { carePayroll } = this.state;
        ...
        <View style={styles.container}>
          <ScrollView>
            ...
            <View style={{ flexDirection: "row" }}>
              ...
              >
                ${carePayroll ? parseInt(carePayroll).toFixed(2) : "11"}/Hr.
              </Text>
            </View>
